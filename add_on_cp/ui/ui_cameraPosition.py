...
class CP_PT_Info(Panel):
    ...
    def draw(self, context):
        ...
        if cp_prop.showPreview:
            ...
            row.template_list(CP_UL_LayerItems.__name__, "abc", cp_prop, "layers", cp_prop,
                              "layers_index", sort_reverse=False, sort_lock=False, maxrows=1, columns=num_cols,
                              type="GRID")
        else:
            row.template_list(CP_UL_LayerItems.__name__, "abc", cp_prop, "layers", cp_prop,
        ...
        row1.prop(cp_prop, 'render_extended_settings', text='', emboss=False,
                  icon_value=4 + int(cp_prop.render_extended_settings))
        ...
        if cp_prop.render_extended_settings:
            ...
            if texture_selected:
                ...
                row.operator("pde.toggle_render_view", text="", emboss=False,
                             icon="RESTRICT_VIEW_OFF").imageName = layer.renderResultSelected.name
        ...
        box = row.box()
        row1 = box.row()
        row1.prop(cp_prop, 'dn_extended_settings', text='', emboss=False,
                  icon_value=4 + int(cp_prop.dn_extended_settings))
        ...
        if cp_prop.dn_extended_settings:
            ...
            if not layer.depthMap:
                row.enabled = False
            row.operator("pde.toggle_render_view", text="", emboss=False,
                         icon="RESTRICT_VIEW_OFF").imageName = layer.depthMap.name if layer.depthMap else ""
            row = box1.row()
            ...
            if not layer.normalMap:
                row.enabled = False
            row.operator("pde.toggle_render_view", text="", emboss=False,
                         icon="RESTRICT_VIEW_OFF").imageName = layer.normalMap.name if layer.normalMap else ""
            row = box2.row()
        ...
        row1 = box.row()
        layer = cp_prop.layers[cp_prop.layers_index]
        row1.prop(cp_prop, 'tex_extended_settings', text='', emboss=False,
                  icon_value=4 + int(cp_prop.tex_extended_settings))
        ...
        if cp_prop.tex_extended_settings:
            ...
            row.operator("pde.toggle_render_view", text="", emboss=False,
                         icon="RESTRICT_VIEW_OFF").imageName = texture_selected.name if texture_selected else ""
        ...
        row1 = box.row()
        layer = cp_prop.layers[cp_prop.layers_index]
        row1.prop(cp_prop, 'debug_extended_settings', text='', emboss=False,
                  icon_value=4 + int(cp_prop.debug_extended_settings))
